...
//     glTexParameteri(specs.target, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(specs.target, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(specs.target, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
void Texture::save_png(const char* filename)
{
...
void Texture::save_fits_greyscale(const char *filename, FitsHeader &header)

{
    ...
    axis_dim[0] = specs.width;
    axis_dim[1] = specs.height;
    num_axis = 2;
    ...
    // TODO: match data type to the specs.type !!
    float *data = new float[specs.width * specs.height * channels];
	float *fits_data = new float[specs.width * specs.height];

    bind();
    glGetTexImage(specs.target, 0, specs.format, specs.type, data);

    for (int i = 0; i < specs.width * specs.height; i++)
        fits_data[i] = data[i*channels];
